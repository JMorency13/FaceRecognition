...
class EmotionDetector:
    ...
    def process_frame(self, frame):
        ...
        for (x, y, w, h) in faces:
            ...
            if self.recommendation_active and (time.time() - self.start_time) >= 60:
                ...
                if recommended_video:
                    self.recommended_videos[emotion].append(recommended_video)
                    ...
                    if feedback == "yes":
                        self.recommendation_engine.update_user_preferences(self.user_id, emotion, "like")
                    elif feedback == "no":
                        self.recommendation_engine.update_user_preferences(self.user_id, emotion, "dislike")
                self.start_time = time.time()
                self.recommendation_active = True
    ...
    def display_recommendation_summary(self):
        """
        Display a summary of recommended videos based on detected emotions
        """
        print("\nRecommendation Summary:")
        for emotion, videos in self.recommended_videos.items():
            print(f"\nEmotion: {emotion}")
            for video in videos:
                print(f"  - {video}")
